...
$query = mysqli_query($con, "SELECT * FROM `sales` INNER JOIN modes_of_payment_tb on modes_of_payment_tb.name=sales.modeofpayment WHERE sales_id = (SELECT MAX(sales_id) FROM sales WHERE user_id='$id' ) AND user_id='$id'  ") or die(mysqli_error($con));

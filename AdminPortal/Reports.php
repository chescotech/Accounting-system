...
$query3 = mysqli_query($con, "SELECT description,date, sum(amount) as total_exp FROM expenses_tb WHERE date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)  group by description ") or die(mysqli_error($con));
$query4 = mysqli_query($con, "SELECT sum(amount) as total_exp FROM `expenses_tb` WHERE date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)") or die(mysqli_error($con));
$query5 = mysqli_query($con, "SELECT sales.date_added, sales_details.qty, sales_details.price FROM sales INNER JOIN sales_details ON sales_details.sales_id = sales.sales_id WHERE date_added >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)") or die(mysqli_error($con));
$query6 = mysqli_query($con, "SELECT qty, price, date2collect FROM draft_temp_trans") or die(mysqli_error($con));
$query7 = mysqli_query($con, "SELECT qty, price, date2collect FROM draft_temp_trans WHERE date2collect < '$currentDate'") or die(mysqli_error($con));
